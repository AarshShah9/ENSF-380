...
public class Translator implements Serializable {
  private final static Pattern langRegionCodePattern = Pattern.compile("([a-z]{2})-([A-Z]{2})");
  private String langRegionCode;
  ...
  public Translator(String langRegionCode) throws IllegalArgumentException, ArgFileNotFoundException {
    ...
    Matcher codeMatcher = langRegionCodePattern.matcher(langRegionCode);

    if (codeMatcher.matches()) {
      this.langRegionCode = langRegionCode;
    } else {
      throw new IllegalArgumentException("Command-line argument invalid.");
  ...
  public void importTranslation() throws IOException, ArgFileNotFoundException {
    File file = new File(this.langRegionCode + ".ser");
    if (file.exists()) {
      deserialize();
    } else {
      importFromText();
  ...
  public void importFromText() throws IOException, ArgFileNotFoundException {
    File file = new File(this.langRegionCode + ".txt");
    if (!file.exists()) {
    ...
    br.close();
    TranslationText translation = new TranslationText(months, days, sentence);
    this.translation = translation;
    serialize();
  }

  public void serialize() throws IOException {
    FileOutputStream fileOut = new FileOutputStream(this.langRegionCode + ".ser");
  ...
  public void deserialize() throws IOException {
    try {
      FileInputStream fileIn = new FileInputStream(this.langRegionCode + ".ser");
  ...
  public String translate(int month, int day, int year) throws IllegalArgumentException {
    ...
    String formattedString = new String(
        String.format(this.translation.getSentence(),
            this.translation.getDays()[day - 1],
            this.translation.getMonths()[month - 1],
            year));
